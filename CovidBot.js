...
bot.on('message', function(msg) {
    ...
    bot.on('message', function(msg1) {
        ...
        if (msg1.text == 'a' || msg1.text == 'A') {
            ...
            bot.sendMessage(msg.chat.id, '\n \n Give the country name to find the details of cases till now');
            x = 'b'
            msg1.text = x
            bot.on('message', function(msg2) {
                msg1.text = x
            ...
            bot.sendMessage(msg.chat.id, '\n \n Give the city name to find the local hospitals');
            x = 'c'
            msg1.text = x
            bot.on('message', function(msg2) {
                msg1.text = x
            ...
            update.message.reply_text("message", quote = False)
            msg1.text = x
        } else if (!((msg1.text >= 'A' && msg1.text <= 'C') || (msg1.text >= 'a' && msg1.text <= 'c'))) {
            bot.sendMessage(msg.chat.id, "\n")
        }
    })
})
...
function chatbot(msg1text, msgchatid, msg2text) {
    console.log("msg1.text, msg.chat.id, msg2.text" + msg1text + msgchatid + msg2text)
    if (msg1text == 'a' || msg1text == 'A') {
        if (msg2text >= 1 && msg2text <= 25) {
            ...
            bot.sendMessage(msgchatid, "Enter 'A' again and give valid number");
        }
        ...
        covidcases.covidcases(msg2text, function(error, locjson) {
            try {
                ...
                bot.sendMessage(msgchatid, "Enter 'B' again and give vaild Country name")
                console.log(error)
            }
        })
        ...
        getLatLong.geoloc(msg2text, function(error, locjson) {
            try {
                                                    ...
                                                    bot.sendLocation(chat_id = msgchatid, latitude = latlong[3].lat, longitude = latlong[3].lon).then(() => {
                                                        bot.sendMessage(msgchatid, names[3]).then(() => {
                                                            bot.sendLocation(chat_id = msgchatid, latitude = latlong[4].lat, longitude = latlong[4].lon).then(() => {
                                                                bot.sendMessage(msgchatid, names[4])
                ...
                bot.sendMessage(msgchatid, "Enter 'C' again and give vaild City name")
                console.log(error)
